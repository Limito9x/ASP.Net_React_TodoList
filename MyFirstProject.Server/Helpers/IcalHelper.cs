...
namespace MyFirstProject.Server.Helpers
{
    public static class IcalHelper
    {
        ...
        /// </summary>
        public static RecurrencePattern ToRecurrencePattern(RecurrenceRule myRule)
        {
            if (myRule == null)
            ...
            if (myRule.DaysOfWeek != null && myRule.DaysOfWeek.Any())
            {
                pattern.ByDay = myRule.DaysOfWeek
                    .Select(d => new WeekDay((DayOfWeek)d))
                    .ToList();
            }
            ...
            if (myRule.EndDate.HasValue)
            {
                pattern.Until = new CalDateTime(myRule.EndDate.Value);
            }

            // End Condition: Count
            if (myRule.OccurrenceCount.HasValue && myRule.OccurrenceCount.Value > 0)
            {
                pattern.Count = myRule.OccurrenceCount.Value;
            }
            ...
            int maxResults = 1000)
        {
            ...
            try
            {
                var pattern = ToRecurrencePattern(rule);
                
                // ✅ QUAN TRỌNG: Đảm bảo luôn có endpoint
                var searchEnd = endAnchor ?? startAnchor.AddYears(1);
            ...
            RecurrenceRule rule)
        {
            ...
            try
            {
                // Tìm trong khoảng hợp lý (1 năm hoặc đến EndDate)
                var searchEnd = rule.EndDate ?? afterDate.AddYears(1);
                
                // Lấy occurrences, giới hạn 100 để tìm cái tiếp theo
                var occurrences = GetOccurrences(startAnchor, searchEnd, rule, maxResults: 100);
