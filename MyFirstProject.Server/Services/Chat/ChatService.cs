...
using MyFirstProject.Server.Plugins;
using MyFirstProject.Server.Services.UserService;
using System.ComponentModel;
using System.Text;
using System.Text.Json;

namespace MyFirstProject.Server.Services.Chat
{
    public interface IChatService
    {
        ...
        public Task CreateSession(SessionDto sessionDto);
    ...
    public class ChatService: IChatService
    {
        ...
        {
            var session = _mapper.Map<Models.ChatSession>(sessionDto);
    ...
    If the user asks to create a plan (e.g., 'plan for IELTS', 'travel plan'), you MUST NOT just describe it.
    You MUST call the 'RenderPlan' function with the detailed JSON data.
    
    Do NOT ask for confirmation. Just generate the plan and call the function immediately.
");
        ...
        {
            // Save User Msg
            _context.ChatMessages.Add(new Models.ChatMessage{ SessionId = sessionId, Role = "user", Content = userMsg });

            // Save AI Msg
            var aiMessageEntity = new Models.ChatMessage
            {
                SessionId = sessionId,
                Role = "assistant",
                Content = aiMsg,
