        ...
        {(templates, { remove: removeTemplate }) => (
          ...
          >
            {templates.map((template, index) => {
              const isSelected =
                selectedInstance?.id ===
                form.getFieldValue(["data", template.name, "id"]);
              return (
                <Card
                  key={template.key}
                  title={`Template ${
                    form.getFieldValue("data")[template.name]?.groupName ||
                    ""
                  } (#${index + 1})`}
                  ...
                  onClick={() =>
                    onSelectInstance &&
                    onSelectInstance(
                      form.getFieldValue(["data", template.name])
                    )
                  }
                            ...
                            <Form.List name={[row.name, "fields"]}>
                              {(fields, { remove: removeField }) => (
                                <Row gutter={16}>
                                  {fields.map((field) => {
                                    const fieldPath = [
                                      "data", // Bắt đầu từ gốc là metadata
